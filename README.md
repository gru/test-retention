
# Kafka Retention Test Stand

Этот стенд предназначен для проверки поведения Apache Kafka при настройке **retention.bytes**, то есть автоматического удаления старых сегментов лога при превышении лимита размера топика — даже если consumer ещё не прочитал эти данные.

## 📁 Состав файлов

### docker-compose.yaml
Поднимает Kafka и Zookeeper. Ключевые параметры:

- `KAFKA_LOG_RETENTION_BYTES = 1048576` (1 МБ)
- `KAFKA_LOG_SEGMENT_BYTES = 1048576` (1 МБ сегменты)
- `KAFKA_LOG_RETENTION_CHECK_INTERVAL_MS = 5000`
- Логирование удаления сегментов включено через:
  ```
  KAFKA_LOG4J_LOGGERS="kafka.log.LogCleaner:DEBUG,kafka.log.Log:DEBUG"
  KAFKA_LOG4J_ROOT_LOGLEVEL=INFO
  ```

### create-topic.sh
Создаёт тестовый топик test-ret-bytes.

### produce-messages.sh
Генерирует заданный объём данных и отправляет их в Kafka.

Использование:
```
./produce-messages.sh <MB> <topic>
```

### consume-messages.sh
Читает все сообщения из топика и выводит:
- количество сообщений
- общий объём в байтах
- объём в МБ
```
./consume-messages.sh <topic> [messages_per_second] [broker]
```
где
- `messages_per_second = -1` — читать без остановок (режим по умолчанию)
- если указано число N — consumer читает N сообщений, затем делает `sleep 1`  

### grep-kafka-logs.sh
Фильтрует вывод `docker logs` по строкам:
```
./grep-kafka-logs.sh "Deleted"
```

### reset-kafka.sh
Очищает окружение (контейнеры + volumes).

### topic-info.sh
Выводит информацию по топику
```
Topic: test-ret-bytes	TopicId: IksAO-wmREuDXSMg-fMGgg	PartitionCount: 1	ReplicationFactor: 1	Configs: segment.bytes=1048576,retention.bytes=10485760
	Topic: test-ret-bytes	Partition: 0	Leader: 1	Replicas: 1	Isr: 1
```

---

## 🧪 Сценарий теста №1

### 1. Сбросить окружение
```
./reset-kafka.sh
docker compose up -d
```

### 2. Создать топик
```
./create-topic.sh
```

### 3. Записать 10 МБ данных
```
./produce-messages.sh 10 test-ret-bytes
```

### 4. Дождаться удаления сегментов
```
./grep-kafka-logs.sh "Deleted"
```

Ожидаемые строки:
```
Deleted log ... .log.deleted
Deleted offset index ...
Deleted snapshot ...
```

### 5. Прочитать данные из топика
```
./consume-messages.sh test-ret-bytes
```

Ожидаемый результат:
- Прочитано ~1 МБ данных
- Остальное Kafka удалила

---

## 🧪 Сценарий теста №2

### 1. Сбросить окружение
```
./reset-kafka.sh
docker compose up -d
```

### 2. Создать топик
```
./create-topic.sh
```

### 3. Запустить consumer в режиме чтения по 100 сообщений
```
./consume-messages.sh test-ret-bytes 100
```

### 4. Запустить producer записав около 15МБ данных
```
./produce-messages.sh 15 test-ret-bytes
```

### 5. Consumer начнет читать данные
На чтение всех сообщений ему должно потребоваться чуть больше 2 минут. 
Видим что consumer прочитал около 9 МБ данных.
```
Messages consumed: 9230
Total bytes consumed: 9460750 bytes
Total size: 9,02 MB
```

### 6. Проверяем что было удаление данных в логе kafka
```
./grep-kafka-logs.sh "Deleted"
```

Ожидаемые строки:
```
Deleted log ... .log.deleted
Deleted offset index ...
Deleted snapshot ...
```

## 🧪 Сценарий теста №3

### 1. Меняем строки в docker-compose.yaml: 
```
KAFKA_LOG_RETENTION_BYTES: 10485760   # 10 МБ retention
KAFKA_LOG_SEGMENT_BYTES: 10485760      # 10 МБ сегменты
```

### 2. Сбросить окружение
```
./reset-kafka.sh
docker compose up -d
```

### 3. Создать топик
```
./create-topic.sh
```

### 4. Запустить consumer в режиме чтения по 100 сообщений
```
./consume-messages.sh test-ret-bytes 100
```

### 5. Запустить producer записав около 10МБ данных
```
./produce-messages.sh 10 test-ret-bytes
```

### 6. Ждем пока consumer дочитает топик до конца
Проверяем что все сообщения прочитаны:
```
Messages consumed: 10240
Total bytes consumed: 10496000 bytes
Total size: 10,01 MB
```

## 🎯 Итог

Kafka удаляет данные **независимо от того, были ли они прочитаны**, если превышен `retention.bytes`.

